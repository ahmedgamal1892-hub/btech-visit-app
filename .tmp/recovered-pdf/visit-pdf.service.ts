...
import {
  createMaxLineWrapOverflowHandler,
  PRODUCT_NAME_MAX_LINES,
} from './pdf-text-truncation'

const PAGE_WIDTH = 210
const PAGE_HEIGHT = 297
const MARGIN = 14
const CONTENT_WIDTH = PAGE_WIDTH - MARGIN * 2
const INSPECTION_TABLE_COLUMN_WIDTHS = {
  brand: CONTENT_WIDTH * 0.14,
  product: CONTENT_WIDTH * 0.5,
  status: CONTENT_WIDTH * 0.12,
  notes: CONTENT_WIDTH * 0.24,
} as const
...
function applyArabicAutoTableCell(data: CellHookData, raw: unknown) {
...
function addSectionTitle(doc: jsPDF, cursor: PdfCursor, title: string) {
  ...
  doc.setFont(PDF_FONT_LATIN, 'bold')
  doc.setFontSize(12)
...
function addParagraph(doc: jsPDF, cursor: PdfCursor, text: string) {
  const alignment = getPdfTextAlignment(text)

  doc.setFont(getPdfFontFamily(text), 'normal')
  doc.setFontSize(10)
  ...
  doc.text(
    text,
    alignment === 'right' ? PAGE_WIDTH - MARGIN : MARGIN,
    cursor.y,
    {
      maxWidth: CONTENT_WIDTH,
      ...getPdfTextOptions(text),
    },
  )
  ...
  autoTable(doc, {
    ...
    didParseCell(data) {
      if (data.section === 'body' && data.column.index === 1) {
        applyArabicAutoTableCell(data, data.cell.raw)
      }
    },
  })
...
function drawInspectionItems(
  ...
  const inspectionProductFontSize = 9
  const wrapProductName = createMaxLineWrapOverflowHandler(doc, {
    fontSize: inspectionProductFontSize,
    maxLines: PRODUCT_NAME_MAX_LINES,
    getFontFamily: getPdfFontFamily,
  })
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      0: { cellWidth: INSPECTION_TABLE_COLUMN_WIDTHS.brand },
      1: {
        cellWidth: INSPECTION_TABLE_COLUMN_WIDTHS.product,
        overflow: wrapProductName,
      },
      2: { cellWidth: INSPECTION_TABLE_COLUMN_WIDTHS.status },
      3: { cellWidth: INSPECTION_TABLE_COLUMN_WIDTHS.notes },
    },
    didParseCell(data) {
      if (data.section === 'body' && [0, 1, 3].includes(data.column.index)) {
        applyArabicAutoTableCell(data, data.cell.raw)
      }
