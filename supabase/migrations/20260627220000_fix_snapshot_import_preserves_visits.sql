-- Fix confirm_snapshot_import: preserve visit rows when replacing snapshot stores.
--
-- Root cause: visits.store_id REFERENCES stores(id) ON DELETE SET NULL caused
-- PostgreSQL to UPDATE submitted visits when stores were deleted, which was
-- blocked by prevent_submitted_visit_mutation.

ALTER TABLE public.visits
  DROP CONSTRAINT IF EXISTS visits_store_id_fkey;
...
COMMENT ON FUNCTION public.confirm_snapshot_import(
  ...
  'Atomically replaces the operational snapshot. Upserts stores by name so visit rows are never updated.';
