...
function drawHeader(doc: jsPDF, details: VisitDetails, cursor: PdfCursor) {
  ...
  doc.roundedRect(MARGIN, cursor.y - 2, 20, 20, 2.5, 2.5, 'F')
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(7.5)
  doc.setTextColor(...COLORS.white)
  doc.text('LOGO', MARGIN + 10, cursor.y + 8, { align: 'center' })

  doc.setFont('helvetica', 'bold')
  doc.setFontSize(15)
  doc.setTextColor(...COLORS.textPrimary)
  doc.text('Visit APP By Gimi', MARGIN + 26, cursor.y + 4)

  doc.setFont('helvetica', 'normal')
  doc.setFontSize(11)
  doc.setTextColor(...COLORS.textMuted)
  doc.text('Visit Report', MARGIN + 26, cursor.y + 11)
...
function drawFooters(doc: jsPDF, generatedAt: string) {
  ...
  for (let page = 1; page <= pageCount; page += 1) {
    ...
    doc.setFontSize(8)
    doc.setTextColor(...COLORS.textMuted)
    doc.text('Generated by Visit APP By Gimi', MARGIN, footerY - 3)
